...
public class PrimitiveRenderer implements ReleasableResource {

    private Mat4f viewProj;

    private Shader shader;

    public PrimitiveRenderer(Mat4f view, Mat4f proj) {
        viewProj = proj.mult(view);
    }

    public void render(Primitive primitive) {
        List<Vec3f> vertices = primitive.vertices();
        Vec3f color = primitive.color();
        ...
        glDrawArrays(primitive.renderingMode(), 0, vertices.size());
